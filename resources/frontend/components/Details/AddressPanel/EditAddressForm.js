...
import styles from "./EditAddressForm.module";
import {
    Box,
    Button,
    Collapse,
...
import { editAddress } from "../../../ajax/backend";
...
export default function EditAddressForm({
    ...
    const {
        ...
        mode: "onChange",
        defaultValues: {
            default: editAddressItem?.deliveryIsDefault,
            fullName: editAddressItem?.deliveryFullName,
            phoneNumber: editAddressItem?.deliveryPhoneNo,
            address: editAddressItem?.deliveryAddress,
            addressInfo: editAddressItem?.deliveryAddressExtra || "",
            postalCode: editAddressItem?.deliveryPostalCode,
            region: parseInt(editAddressItem?.deliveryRegion),
            province: parseInt(editAddressItem?.deliveryProvince),
            municipality: parseInt(editAddressItem?.deliveryCity),
            barangay: parseInt(editAddressItem?.deliveryBarangay),
        },
    });

    useEffect(() => {
        if (editAddressOpen && editAddressItem) {
            reset({
                default: editAddressItem?.deliveryIsDefault,
                fullName: editAddressItem?.deliveryFullName,
                phoneNumber: editAddressItem?.deliveryPhoneNo,
    ...
    }, [editAddressOpen, editAddressItem, reset]);
    ...
    const onAddressSubmit = (formState) => {
        editAddress(editAddressItem?.orderDeliveryID, formState, (response) => {
    ...
    return (
        <Collapse in={editAddressOpen}>
            <Box
                className={
                    page === "details"
                        ? styles.editAddressContainerDetails
                        : styles.editAddressContainerCheckout
                }
                component="form"
                onSubmit={handleSubmit(onAddressSubmit)}
            >
                {editAddressItem?.deliveryIsDefault === 1 ? null : (
                    <FormControlLabel
                        className={styles.editAddressSwitch}
                        control={
                            <Controller
                                name="default"
                                control={control}
                ...
                <Box className={styles.editAddressButtonContainer}>
                    <Button
                        className={styles.editAddressCancel}
                        onClick={() => {
                            setEditAddressOpen(false);
                            reset();
                            setFormState({
                    ...
                    <Button className={styles.editAddressSave} type="submit">
                        Save
                    </Button>
                </Box>
            </Box>
